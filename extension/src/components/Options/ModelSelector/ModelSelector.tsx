...
export default function ModelSelector({ options }: { options: Provider[] }) {
    ...
    return (
        <div className={styles.container} ref={containerRef}>
            ...
            {open && (
                <div ref={menuRef} className={`${styles.dropdown} ${ placement === "top" ? styles.dropdownTop : styles.dropdownBottom}`}>
                    <button className={styles.dropdownItem} onClick={() => handleOptionToggle("Choose model")}>Choose model</button>
                    {options.flatMap((provider) =>
                        provider.models.map((model) => (
                        <button className={styles.dropdownItem} onClick={() => handleOptionToggle(`${provider.name} > ${model}`)} key={`${provider.name}-${model}`}>
                            {provider.name} {'>'} {model}
                        </button>
                        ))
                    )}
