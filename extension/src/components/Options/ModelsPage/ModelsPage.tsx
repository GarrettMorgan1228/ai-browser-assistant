...
export default function ModelsPage() {
  ...
  useEffect(() => {
    chrome.storage.local.get(
      ["selected", "modelsByProvider", "apiKeys"],
      (res) => {
        if (res.selected) {
  ...
  function handleAdd(provider: Provider) {
    setSelected((prev) =>
      prev.includes(provider.name) ? prev : [...prev, provider.name]
    );
    setModelsByProvider((prev) => ({
      ...prev,
      [provider.name]: prev[provider.name] ?? provider.models ?? [],
    }));
  }
  ...
  const chosen = selected
    .map((n) => providerCatalog.find((p) => p.name === n)!)
    .filter(Boolean);
      ...
      <div className={styles["models-item"]}>
        ...
        <div>
          <button
            ...
            disabled={!changed}
            onClick={() => onSave(draftModels, draftKey)}
          >
            Save
          </button>
          <button
...
function SelectedProviders({
  ...
  setModelsByProvider: React.Dispatch<
...
function ModelSettings({
  ...
  const options = selected.map((name) => ({
    ...providerCatalog.find((p) => p.name === name)!,
