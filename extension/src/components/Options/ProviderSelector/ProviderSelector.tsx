...
export default function ProviderSelector({ options, onSelect }: { options: Provider[]; onSelect: (p: Provider) => void }) {
    ...
    <div className={styles.container} ref={containerRef}>
      ...
      {open && (
        <div className={styles.dropdown}>
          {options.length === 0 ? (
            <div className={styles.noOptions}>No more providers to choose from.</div>
          ) : (
            options.map((provider) => (
              <button key={provider.name} className={styles.dropdownItem} onClick={() => choose(provider)}> 
                {provider.name}
              </button>
            ))
          )}
