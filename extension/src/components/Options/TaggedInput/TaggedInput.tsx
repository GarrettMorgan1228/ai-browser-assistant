...
export default function TagsInput({ value, onChange }: Props) {
  ...
  const onKeyDown: React.KeyboardEventHandler<HTMLInputElement> = e => {
    if ((e.key === "Enter" || e.key === " ") && draft.trim()) { // add tag
      e.preventDefault();
      addFromDraft();
    } else if (e.key === "Backspace" && draft.length === 0 && value.length > 0) { // remove last tag
      e.preventDefault();
  ...
  return (
    <div className={styles["tags-field"]}>
      ...
      <p className={styles["tags-help"]}>Type and press Enter or Space to add.</p>
    </div>
  );
}
